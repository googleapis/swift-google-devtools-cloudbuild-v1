...
Add `swift-google-devtools-cloudbuild-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-devtools-cloudbuild-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudBuildV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudBuildV1 <target-name> --package swift-google-devtools-cloudbuild-v1
```
